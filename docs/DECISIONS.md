...
**Amendement du 2026-08-02 — le mode d'exploitation ne devient pas une colonne (grilling du porteur).**
Le tarif CDTS ne dépend pas que du sens : cinq lignes du document portent un mode d'exploitation dans leur libellé — `EXP02B` (Ligne Régulière), `EXP15` (Tramping), `EXP16` (LR conteneurisé), `EXP17` (Tramping conteneurisé), face à `EXP02` sans mention. **Toutes concernent le bois, et uniquement à l'export** : aucun conteneur, véhicule ou vrac n'est concerné, et l'import ne l'est nulle part.
**Décision :** pas de colonne `mode_exploitation` sur `bareme_lignes`. Le mode reste où le document le met — dans la désignation — et les variantes restent des lignes distinctes. Motif du porteur : le cas ne touche que le bois et se présente très rarement ; une colonne sur 58 lignes dont 5 la renseigneraient est disproportionnée.
**Réserve consignée :** le jour où le calcul sera automatique, le moteur devra choisir entre `EXP02B` et `EXP15` selon le mode de **l'escale** (et non du navire — voir la règle du mode d'exploitation). Sans colonne, il le déduirait du texte « (TRAMPING) », ce qui est fragile. À rouvrir au dépouillement, avec le besoin réel sous les yeux ; une colonne s'ajoute en une migration.
**⚠ NON TRANCHÉ — bloquant pour la facturation du bois :** trois références coexistent pour « bois en grume et produits dérivés » à l'export — `EXP02` à 1 773,70, `EXP02B` (ligne régulière) à 1 569,04 et `EXP15` (tramping) à 1 909,00. Si tout bois part soit en ligne régulière soit en tramping, `EXP02` n'est jamais applicable. **Le porteur ne sait pas encore laquelle le CGC retient** ; question posée le 2026-08-02, réponse en attente. Les trois lignes restent en base jusque-là.
