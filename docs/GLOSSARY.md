...
| Compte client | Compte d'un tiers du portail — titulaire d'un consignataire ou agent consignataire — par opposition au **compte interne** d'un agent du CGC | `users` (mêmes comptes, distingués par leur rattachement) | Porte l'un des **deux rôles clients figés** du catalogue — `Consignataire — compte maître` ou `Agent consignataire` (ADR-0031, qui amende ADR-0025). Le rôle dit *ce qu'il peut faire* ; son **rattachement** (société, armements affectés) dit *sur quoi*. Création et validation réservées à l'Administrateur (`comptes-clients.gerer`) |
| Espace client | La partie du portail que voit un compte client : sa société est nommée dans l'en-tête (« Espace de {société} »), et sa navigation ne contient que ce à quoi il a droit | — | Ce n'est **pas** une route à part : tous les comptes arrivent sur le même tableau de bord, seuls la navigation et le contexte diffèrent (ADR-0030) |
| Recomposer un rôle | Modifier la liste des permissions portées par un rôle existant. **N'est pas** créer, renommer ou supprimer un rôle : le catalogue des rôles reste celui du code | `syncPermissions()` (spatie/laravel-permission) | Geste réservé à l'Administrateur (`roles.gerer`). Trois colonnes ne sont pas recomposables : Administrateur, qui porte par définition tout ce qui a un sens côté CGC, et les deux rôles clients, qui suivent la position occupée dans une société (ADR-0025, ADR-0031) |
| Conférer un rôle | L'attribuer à un compte. Un compte ne confère qu'un rôle dont il porte déjà **toutes** les permissions — « on ne donne pas ce qu'on n'a pas » | `User::peutConferer()` | Vaut aussi pour le **retrait** : on ne modifie pas un compte dont on ne pourrait pas conférer les rôles, sinon on rétrograderait un Administrateur ou on prendrait sa place (ADR-0033). À ne pas confondre avec **recomposer un rôle**, qui change son contenu et non son porteur |
| Nomenclature CGC | Grille des catégories tarifaires CDTS du CGC (cf. `BAREME-CDTS.md`) | — | Le « passage à la nomenclature » = catégorisation des lignes du manifeste. **Ne figure pas au barème** : le code seul n'est pas parlant et les libellés des catégories restent à recueillir (ADR-0034) |
| Ligne de barème | Un article tarifé du barème CDTS : une référence (`EXP01`, `IMP011`), une désignation et un montant | `BaremeLigne` | Le montant est tenu **en francs CFA**, comme le document officiel ; l'euro se déduit de la parité fixe et ne se saisit pas. Une ligne **désactivée** sort de l'exploitation sans quitter la grille — la **supprimer** l'efface. Les remises et « taux appliqués » du papier ne sont pas gérés par la plateforme (ADR-0034) |
| Référence article (`EXP01`, `IMP011`) | Le code sous lequel le CGC connaît cet article **dans son SAGE**. Ce n'est pas un identifiant e-CDTS : la plateforme le reprend tel quel pour que les deux systèmes parlent du même article | `bareme_lignes.reference` | D'où les irrégularités qu'on ne corrige pas : `EXP02B` intercalé, `IMP011` et `IMP012` là où l'on attendrait `IMP11` |
| Sens du trafic | Export ou import — la marchandise sort du pays ou y entre | `SensTrafic` | Le barème en tient deux volets distincts : la même marchandise n'est pas tarifée pareil selon qu'elle part ou qu'elle arrive |
