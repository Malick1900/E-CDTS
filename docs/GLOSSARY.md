...
| Nomenclature CGC | Grille des catégories tarifaires CDTS du CGC (cf. `BAREME-CDTS.md`) | `bareme_lignes.nomenclature` | Le « passage à la nomenclature » = catégorisation des lignes du manifeste. Codée sur deux chiffres, `01` à `07` ; les libellés des sept catégories ne figurent pas sur le document officiel et restent à recueillir |
| Ligne de barème | Un article tarifé du barème CDTS : une référence (`EXP01`, `IMP011`), une désignation, une catégorie de nomenclature et un montant | `BaremeLigne` | Le montant est tenu **en francs CFA**, comme le document officiel ; l'euro se déduit de la parité fixe et ne se saisit pas. Une ligne **désactivée** sort de l'exploitation sans quitter la grille — la **supprimer** l'efface. Les remises et « taux appliqués » du papier ne sont pas gérés par la plateforme (ADR-0034) |
