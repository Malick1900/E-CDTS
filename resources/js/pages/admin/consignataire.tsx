...
const compte = (n: number, [un, plusieurs]: [string, string]) =>
    `${n} ${n > 1 ? plusieurs : un}`;
...
const IconeSociete = (
    ...
    >
        ...
        />
        <path
            d="M8 8V5.5A1.5 1.5 0 0 1 9.5 4h5A1.5 1.5 0 0 1 16 5.5V8M9 12h2M13 12h2M9 15.5h2M13 15.5h2"
            stroke="#1D3E9C"
            strokeWidth="1.5"
            strokeLinecap="round"
        />
...
const IconePort = (
    ...
    >
        ...
        <path
            d="M10 6v11M6 10h8M3.6 12.4a6.4 6.4 0 0 0 12.8 0"
            stroke="#1D3E9C"
            strokeWidth="1.5"
            strokeLinecap="round"
        />
    ...
    return (
        ...
        >
            ...
            {fiche.titulaire_user_id === null ? (
                <span style={{ fontSize: 12.5, color: '#B26200' }}>
                    À désigner — personne ne peut créer les comptes des agents.
                </span>
            ) : (
                <div
                    style={{
                ...
                >
                    ...
                    >
                        {fiche.titulaire_name}
                    </span>
                    <span style={{ fontSize: 11.5, color: '#8A93A6' }}>
                        {fiche.titulaire_email}
                    </span>
                    {fiche.titulaire_job_title && (
                        <span style={{ fontSize: 11.5, color: '#8A93A6' }}>
                            · {fiche.titulaire_job_title}
                        </span>
                    )}
...
export default function ConsignataireFichePage({
    consignataire,
    armements = [],
    ports = [],
    agents = [],
    ...
    return (
        <>
            <Head
                title={`${consignataire.name} — Utilisateurs & habilitations`}
            />
            <AdminShell
                module="users"
            ...
            >
                ...
                >
                    ...
                    <FicheEntete
                        ...
                        metas={[
                            consignataire.sigle,
                            consignataire.pays_immatriculation_name,
                            consignataire.rccm_nif,
                            consignataire.email,
                            consignataire.telephone,
                        ]}
                    ...
                    >
                        <Titulaire
                            fiche={consignataire}
                            peutGerer={peutGerer}
                            onRemplacer={() => setRemplace(true)}
                        />
                    </FicheEntete>

                    <BlocFiche
                        titre="Armements représentés"
                        compte={compte(armements.length, [
                            'armement',
                            'armements',
                        ])}
                        note={
                            <>
                                ...
                                : un armement peut être représenté par plusieurs
                                sociétés, et les agents de celle-ci n’opèrent
                                que sur les armements ci-dessus. Le rattachement
                                se modifie depuis « Modifier » dans la liste des
                                consignataires.
                            </>
                        }
                    >
                        {armements.length === 0 ? (
                            <BlocVide>
                                Aucun armement représenté — cette société ne
                                peut consigner aucune escale.
                            </BlocVide>
                        ) : (
                            armements.map((a) => (
                                <CarteRattachement
                                    key={a.id}
                                    pastille={
                                        <div style={pastilleStyle}>
                                            {(
                                                a.sigle ?? a.name.slice(0, 3)
                                            ).toUpperCase()}
                                        </div>
                                    }
                                    titre={a.name}
                    ...
                    <BlocFiche
                        titre="Ports de rattachement"
                        compte={compte(ports.length, ['port', 'ports'])}
                    >
                        {ports.length === 0 ? (
                    ...
                    >
                        {agents.length === 0 ? (
                            ...
                            >
                                Aucun compte agent.{' '}
                                {consignataire.titulaire_user_id === null
                                    ? 'Désignez d’abord un titulaire : c’est lui qui les crée.'
                                    : 'Le titulaire les crée depuis son espace ; le CGC les valide.'}
                            </div>
                        ) : (
