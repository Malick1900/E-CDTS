...
const VIERGE: Form = { name: '', imo: '', type_navire_id: null, armement_id: null, pays_id: null, mode_exploitation_defaut: 'ligne_reguliere' };
    ...
    return (
        <>
            <TableCard
                ...
                largeurMin={1180}
                vide={ref.recherche ? 'Aucun navire ne correspond à la recherche.' : 'Aucun navire — ajoutez-en un avec « Nouveau navire ».'}
                page={ref.page}
                parPage={PAR_PAGE}
                onPage={ref.setPage}
                apercu={
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#5A6478' }}>
                        <span style={{ width: 11, height: 11, borderRadius: 3, background: '#7EC8F0', flex: 'none' }} />
                        Type &amp; mode d’exploitation : lus par le moteur de calcul
                    </span>
                }
            ...
            >
                {ref.lignesPage.map((n) => (
                    <tr key={n.id} className="ea-row">
                        ...
                        <Td style={{ borderLeft: '3px solid #7EC8F0', background: '#FBFDFF' }}>
                            {n.type_navire_code ? (
                                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7 }}>
                                    <span style={{ fontSize: 11, fontWeight: 700, color: '#3A4356', background: '#EEF1F7', border: '1px solid #D8DEE9', borderRadius: 5, padding: '2px 7px' }}>{n.type_navire_code}</span>
                                    <span style={{ fontSize: 12, color: '#5A6478' }}>{n.type_navire_name}</span>
                                </span>
                            ) : (
                                <Vide />
                            )}
                        ...
                        <Td style={{ fontVariantNumeric: 'tabular-nums' }}>{n.imo ?? <Vide />}</Td>
                        <Td>
            ...
            {ref.mode && (
                <Drawer
                    titre={ref.mode === 'creation' ? 'Nouveau navire' : 'Modifier le navire'}
                ...
                >
                    <TextField label="Nom du navire" requis placeholder="ex. Maersk Cadiz" valeur={ref.form.name} onChange={(v) => ref.champ('name', v)} erreur={ref.erreurs.name} />
                    <TextField label="Code IMO" chiffres maxLength={7} placeholder="ex. 9412367" valeur={ref.form.imo} onChange={(v) => ref.champ('imo', v)} erreur={ref.erreurs.imo} aide="Sept chiffres. Facultatif : tous les navires n’ont pas d’IMO." />
                    <SelectField label="Type de navire" valeur={ref.form.type_navire_id} onChange={(v) => ref.champ('type_navire_id', v)} options={optionsTypes} erreur={ref.erreurs.type_navire_id} />

                    <Field label="Mode d’exploitation par défaut" aide="Recopié sur l’escale à sa création ; seul le CGC peut l’y modifier." erreur={ref.erreurs.mode_exploitation_defaut}>
                        <div style={{ display: 'flex', gap: 8 }}>
                            {MODES.map((mode) => {
                                const actif = ref.form.mode_exploitation_defaut === mode;

                                return (
                                    <button
                                        key={mode}
                                        type="button"
                                        onClick={() => ref.champ('mode_exploitation_defaut', actif ? null : mode)}
                                        style={{
                                            flex: 1, height: 36, borderRadius: 6, cursor: 'pointer', fontSize: 12.5, fontWeight: 700,
                                            border: `1px solid ${actif ? '#1D3E9C' : '#D8DEE9'}`,
                                            background: actif ? '#EEF3FF' : '#fff',
                                            color: actif ? '#1D3E9C' : '#5A6478',
                                        }}
                                    >
                                        {MODE_LABEL[mode]}
                                    </button>
                    ...
                    <SelectField label="Armement rattaché" valeur={ref.form.armement_id} onChange={(v) => ref.champ('armement_id', v)} options={optionsArmements} erreur={ref.erreurs.armement_id} />
                    <SelectField label="Pavillon" valeur={ref.form.pays_id} onChange={(v) => ref.champ('pays_id', v)} options={optionsPays} erreur={ref.erreurs.pays_id} />
                </Drawer>
            )}
