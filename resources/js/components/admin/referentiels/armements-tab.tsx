...
const VIERGE: Form = { name: '', sigle: '', pays_origine_id: null, pays_immatriculation_id: null, gerant: '', rccm_nif: '', adresse: '' };
    ...
    return (
        <>
            <TableCard
                ...
                largeurMin={900}
                vide={ref.recherche ? 'Aucun armement ne correspond à la recherche.' : 'Aucun armement — ajoutez-en un avec « Nouvel armement ».'}
                page={ref.page}
                parPage={PAR_PAGE}
                onPage={ref.setPage}
                ...
                {ref.lignesPage.map((a) => (
                    <tr key={a.id} className="ea-row">
                        ...
                        <Td style={{ fontVariantNumeric: 'tabular-nums' }}>
                            {a.navires_count === 0 ? <Vide /> : `${a.navires_count} navire${a.navires_count > 1 ? 's' : ''}`}
                        </Td>
                        <Td>
            ...
            {ref.mode && (
                <Drawer
                    titre={ref.mode === 'creation' ? 'Nouvel armement' : 'Modifier l’armement'}
                ...
                >
                    <TextField label="Nom de l’armement" requis placeholder="ex. Maersk Line" valeur={ref.form.name} onChange={(v) => ref.champ('name', v)} erreur={ref.erreurs.name} />
                    <TextField label="Sigle" majuscules maxLength={10} placeholder="ex. MSK" valeur={ref.form.sigle} onChange={(v) => ref.champ('sigle', v)} erreur={ref.erreurs.sigle} aide="Abréviation affichée dans les tableaux." />
                    <SelectField label="Pays d’origine" valeur={ref.form.pays_origine_id} onChange={(v) => ref.champ('pays_origine_id', v)} options={optionsPays} erreur={ref.erreurs.pays_origine_id} />
                    ...
                    <TextField label="Gérant" placeholder="Nom du gérant" valeur={ref.form.gerant} onChange={(v) => ref.champ('gerant', v)} erreur={ref.erreurs.gerant} />
